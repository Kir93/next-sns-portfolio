...
#### 3. 파일 구조 (App Router co-location, 현행)

라우트 전용 피드 컴포넌트는 `app/_components/sns/`에 co-locate하고, 라우트 교차 프리미티브만 `src/components/common/`에 둔다.

```bash
app/_components/sns/          # 라우트 전용 피드 컴포넌트
├── FeedContainer.tsx         # Suspense + ErrorBoundary 비동기 경계 (ADR-004)
├── FeedSkeleton.tsx          # 로딩 fallback
├── FeedError.tsx             # 에러 UI (다시 시도)
├── useInfiniteFeed.ts        # useSuspenseInfiniteQuery
├── PostComposer/             # 게시 폼 (RHF + Zod)
└── SnsCard/
    ├── index.tsx             # SnsCard 컴포넌트 Main
    ├── ImageGrid.tsx         # 이미지 레이아웃 로직 (§2.1.3)
    ├── PostHeader.tsx        # 사용자 정보 부분
    ├── PostContent.tsx       # 텍스트 및 이미지 부분
    ├── PostActions.tsx       # 좋아요·리트윗 등 버튼 부분
    └── useToggleLike.ts      # 좋아요 낙관 토글

src/components/common/        # 라우트 교차 프리미티브
├── Avatar.tsx
├── ErrorBoundary.tsx         # 자체 구현 클래스 컴포넌트 (ADR-004)
└── Icon.tsx
```
